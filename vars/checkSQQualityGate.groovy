    ...
    //timeout(time: 120, unit: 'SECONDS') { // Just in case something goes wrong, pipeline will be killed after a timeout
            ...
            withSonarQubeEnv('Sonar') {
                ...
                timeout(time: 15, unit: 'MINUTES') {
                    waitUntil {
                    if (isUnix()) {
                        sh "curl -k $SONAR_CE_TASK_URL -o ceTask.json"
                    } else {
                        bat "curl -k %SONAR_CE_TASK_URL% -o ceTask.json"
                    }
                ...
                if (isUnix()) {
                    ...
                    bat "curl -k $qualityGateUrl -o qualityGate.json"
                }
        ...
        echo qgstatus
        if ((qgstatus == 'WARN')||(qgstatus == 'SUCCESS')||(qgstatus == 'OK')) {
            ...
            setSonarLeakBaselineVersion("${PIPELINE_VERSION}")
        }
